...
说明：
  - 当前 LLM：默认 OpenRouter，模型 id 见 .env 的 OPENROUTER_TEXT_MODEL（默认 openai/gpt-5.4）。支持通过 --use-gemini 切换为 google/gemini-2.5-flash。
...
def _openrouter_env() -> tuple[str, str, str]:
    ...
    vision = os.getenv("OPENROUTER_VISION_MODEL", "openai/gpt-5.4").strip()
...
def _build_board_emoji_sections(
    ...
    *,
    ...
    for row_idx, r in enumerate(rows):
        ...
        if minimal_for_llm:
            short_name = disp_show
            if "（" in disp_show and "）" in disp_show:
                short_name = disp_show.split("（")[1].split("）")[0]
            
            carry_short = "空"
            if eqs:
                carry_short = ",".join(_equip_name_with_optional_short(x, equip_map).split("（")[1].split("）")[0] if "（" in _equip_name_with_optional_short(x, equip_map) else _equip_name_with_optional_short(x, equip_map) for x in eqs)
            
            block = f"{short_name}({star_n}星{int(cost_n)}费/{carry_short})"
            
            audit_str = ""
            if audit_lines:
                real_audits = [line for line in audit_lines if "装备佩戴正常" not in line]
                if real_audits:
                    audit_str = f"{short_name}: " + "；".join(real_audits).replace("❗ ", "").replace("⚠ ", "")
        else:
        ...
        raw.append(
            {
                ...
                "block": block,
                "disp_show": disp_show,
                "short_name": short_name if minimal_for_llm else disp_show,
                "role_tags": role_tags_list,
                "slot_tag": slot_tag,
                "cell_row": cell_row,
                "cell_col": cell_col,
                "eqs": eqs,
                "rec_list": rec_list,
                "audit_str": audit_str if minimal_for_llm else "",
            }
        )

    core_rows = [x for x in raw if x["core"]]
    work_rows = [x for x in raw if not x["core"] and x["slot_tag"] != "挂件"]
    ...
    if minimal_for_llm:
        return [x["block"] for x in core_rows], [x["block"] for x in work_rows], raw
    else:
        # 非极简模式下，保留挂件的展示
        full_work_rows = [x for x in raw if not x["core"]]
        full_work_rows.sort(key=lambda x: (x["grp"], -x["star_n"], -x["cost_n"], x["row_idx"]))
        return [x["block"] for x in core_rows], [x["block"] for x in full_work_rows], raw
...
def _get_piece_tier(piece: Dict[str, Any]) -> str:
    is_core = piece.get("core", False)
    star = piece.get("star_n", 1)
    cost = piece.get("cost_n", 1)
    slot_tag = piece.get("slot_tag", "")
    
    if is_core and star >= 2:
        return "T0"
    if (is_core and star == 1 and cost >= 4) or (not is_core and slot_tag in ("打工仔", "混合") and star >= 2):
        return "T1"
    if (is_core and star < 2) or (not is_core and slot_tag in ("打工仔", "混合") and star < 2):
        return "T2"
    return "T3"

def _build_position_audit(raw: List[Dict[str, Any]]) -> str:
    issues = []
    for p in raw:
        tier = _get_piece_tier(p)
        ...
        tags = p.get("role_tags", [])
        slot_tag = p.get("slot_tag", "")
        name = p.get("disp_show", "?")
        
        is_mage_or_shooter = "法师" in tags or "射手" in tags
        is_tank = "坦克" in tags or "战士" in tags or slot_tag == "主坦"
        
        if is_mage_or_shooter and not is_tank and row <= 2:
            issues.append(f"- {name}（法师/射手）当前在第 {row} 排，存在被秒杀风险，建议移至后排（第3-4排）。")
        elif is_tank and not is_mage_or_shooter and row >= 3:
            issues.append(f"- {name}（坦克）当前在第 {row} 排，无法有效承伤，建议移至前排（第1-2排）。")
            
    if not issues:
        return "站位合理，未见明显错位。"
    return "\n".join(issues)

def _annotate_equipments(items: List[str], raw: List[Dict[str, Any]], equip_map: Dict[str, Any]) -> List[str]:
    annotated = []
    pieces = []
    for p in raw:
        if p.get("slot_tag") == "挂件":
            continue
        slots_used = len(p.get("eqs", []))
        if slots_used < 3:
            # 优先级: T0 > T1 > T2, 同级按 cost_n 降序
            tier = _get_piece_tier(p)
            tier_val = {"T0": 3, "T1": 2, "T2": 1, "T3": 0}.get(tier, 0)
            pieces.append({
                "p": p,
                "slots_used": slots_used,
                "pri": (tier_val, p.get("cost_n", 0))
            })
            
    pieces.sort(key=lambda x: x["pri"], reverse=True)
    ...
    for item_str in items:
        ...
        for pd in pieces:
            ...
            rec_list = pd["p"].get("rec_list", [])
            valid = False
            
            if any(base_name in str(r) for r in rec_list):
                valid = True
            else:
                role_tags = pd["p"].get("role_tags", [])
                slot_role = pd["p"].get("slot_tag", "")
                if _target_equip_name_valid_for_role(base_name, role_tags, slot_role, equip_map):
                    valid = True
                    
            if valid:
                assigned_to = pd["p"].get("disp_show")
                pd["slots_used"] += 1
                break
                
        if assigned_to:
            annotated.append(f"{item_str} => 推荐给：{assigned_to}")
        else:
            annotated.append(item_str)
            
    return annotated

def _format_equip_column_emoji(summary: Dict[str, Any], raw: List[Dict[str, Any]], minimal_for_llm: bool = False) -> str:
    """左侧装备：滞留优先，其次散件与可合成。"""
    ec = (summary.get("modules") or {}).get("equip_column") or {}
    m = ec.get("matches") if isinstance(ec, dict) else None
    if not isinstance(m, list) or not m:
    ...
    cheng_annotated = _annotate_equipments(cheng, raw, equip_map)
    craft_lines_annotated = _annotate_equipments(craft_lines, raw, equip_map)

    def _join(xs: List[str]) -> str:
        return (
            "、".join(_equip_name_with_optional_short(x.split(" =>")[0].strip(), equip_map) + (" =>" + x.split(" =>")[1] if " =>" in x else "") for x in xs) if xs else "无"
        )

    def _join_craft(xs: List[str]) -> str:
        if not xs:
            return "无"
        return "、".join(_craft_line_display_with_short(x.split(" =>")[0].strip(), equip_map) + (" =>" + x.split(" =>")[1] if " =>" in x else "") for x in xs)

    cheng_s = _join(cheng_annotated)
    san_s = _join(san) # 散件不推荐，因为不能佩戴
    craft_s = _join_craft(craft_lines_annotated)
    ...
    if minimal_for_llm:
        ...
        return (
            f"闲置成装: {_strip_brackets(cheng_s)}\n"
            f"闲置散件: {san_s} | 可合成: {_strip_brackets(craft_s)}"
        )
    ...
    *,
    ...
    if minimal_for_llm:
        ...
        for h in missing_current_build:
            # 尝试获取短名
            row_doc = lookup_core_chess_row(h, core_list)
            short_name = h
            if row_doc and isinstance(row_doc, dict):
                ns = str(row_doc.get("name_short") or "").strip()
                if ns:
                    short_name = ns
            ...
            cs = f"{int(cost)}费" if cost is not None else "?费"
            names.append(f"{short_name}({cs})")
        target_block = "、".join(names) if names else "- （无）"
    else:
        for h in missing_current_build:
            cost = chess_cost_from_core_or_legend(h, core_list, legend_map)
    ...
    if minimal_for_llm:
        for g in dc_rows:
            ...
            # 尝试提取括号中的短名，例如 "厄运小姐（女枪）" -> "女枪"
            short_name = disp
            if "（" in disp and "）" in disp:
                short_name = disp.split("（")[1].split("）")[0]
                
            star_seg = str(g.get("star_seg") or "?星")
            discard_lines.append(f"{short_name}({star_seg})")
        discard_block = "、".join(discard_lines) if discard_lines else "- （无）"
    else:
        for g in dc_rows:
            disp = str(g.get("display") or "")
            star_seg = str(g.get("star_seg") or "?星")
...
def build_matchbook_report(
    ...
    *,
    ...
    core_sec, work_sec, raw_board = _build_board_emoji_sections(
        summary,
        core_list,
        summary_json_path=summary_json_path,
        legend_chess_path=DEFAULT_RAG_LEGEND_CHESS,
        minimal_for_llm=minimal_for_llm,
    )
    core_txt = "\n".join(core_sec) if core_sec else "- （无）"
    work_txt = "\n".join(work_sec) if work_sec else "- （无）"
    
    position_audit_txt = _build_position_audit(raw_board)

    if minimal_for_llm:
        # 极简模式专属排版
        all_blocks = core_sec + work_sec
        board_str = ", ".join(all_blocks) if all_blocks else "无"
        
        # 从 raw_board 提取待追 2 星的棋子 (星级<2 的主C/主坦，或者打工仔中星级<2的)
        to_2star = []
        for p in raw_board:
            if p["star_n"] < 2 and p["slot_tag"] != "挂件":
                to_2star.append(p.get("short_name", p["disp_show"]))
        to_2star_str = ", ".join(to_2star) if to_2star else "无"
        
        # 提取装备纠错
        equip_audits = [p["audit_str"] for p in raw_board if p.get("audit_str")]
        equip_audit_str = "；".join(equip_audits) if equip_audits else "无错配"
        
        # 处理闲置装备
        equip_col = _format_equip_column_emoji(summary, raw_board, minimal_for_llm=True)
        
        # 获取 RAG 补充信息
        tb, db = "", ""
        if not no_rag and chess_meta:
            tb, db, _ = _format_chess_target_and_discard(
                summary,
                chess_meta,
                core_list,
                lineup_top_doc=lineup_top_doc,
                target_level_n=target_level_n,
                minimal_for_llm=True,
            )
            
        # 提取阵容推荐
        lineup_title = "无"
        op_advice = "无"
        if not no_rag and lineup_top_doc:
            lineup_title = f"{lineup_top_doc.get('name', '')} ({lineup_top_doc.get('bonds_show', '')})"
            op_advice = str(lineup_top_doc.get("op_advice") or "无")
            
        parts = [
            "[局势]",
            f"{phase}阶段 | {level}级 | {gold}金币 | {hp_self}血",
            "",
            "[资产与升星]",
            f"场上: {board_str}",
            f"待追2星: {to_2star_str}",
            ...
            "",
            "[本地审计指令]",
            f"站位: {position_audit_txt.replace('站位合理，未见明显错位。', '无错位')}",
            equip_col,
            f"装备纠错: {equip_audit_str}",
            f"阵容推荐: {lineup_title}",
            f"运营建议: {op_advice}",
        ]
        return "\n".join(parts)
    ...
    if work_txt and work_txt != "- （无）":
        parts.extend([
            f"🔧{REPORT_EMOJI_GAP}打工棋子",
            work_txt,
            REPORT_SEP,
        ])

    parts.extend([
        f"👁️‍🗨️{REPORT_EMOJI_GAP}站位审计",
        position_audit_txt,
        REPORT_SEP,
        f"🎒{REPORT_EMOJI_GAP}闲置装备",
        _format_equip_column_emoji(summary, raw_board),
    ])
    ...
    if not no_rag and chess_meta:
        ...
        if eqh and not minimal_for_llm:
...
你将收到以下结构化文本作为分析基础：
1. **哈基星提问**：他的弱智原话。
2. **对局情报**：包含 `[局势]`、`[资产与升星]`、`[本地审计指令]` 的极简指令流。

---

# 🚀 输出逻辑控制器 (Priority: A > B > C > D > E)
根据输入情报，**仅输出**最高优先级命中 Trigger 的结论。

### 🔴 Trigger A: 生死判定
- **判定**: `[局势]` 中 `血量` < 25
- **指令**: 下令 **[梭哈]**，督促玩家买出 `待追2星` 里的牌，或者搜出 `待找新牌` 里的核心，强制要求卖掉 `待卖弃子` 。

### 🟡 Trigger B: 节奏诊断
- **判定**: 哈基星问“怎么玩”、“什么节奏”、“要不要拉人口”
- **指令**: 直接复读 `[本地审计指令]` -> `运营建议` 中的原话（并用嘲讽语气包装）。

### 🔵 Trigger C: 资产优化
- **判定**: `待找新牌` 或 `待追2星` 不为"无"。
- **指令**: 下令 **[慢搜]**，找齐 `待找新牌` 或把 `待追2星` 升星，卖掉 `待卖弃子`。

### 🟢 Trigger D: 装备调度
- **判定**: `闲置成装`、`可合成` 存在 "=> 给(某人)" 的建议，或 `装备纠错` 中指出了具体的错配。
- **指令**: 严格执行审计结果，命令玩家把装备给谁，或者喷他装备合错了。

### ⚪ Trigger E: 站位纠偏
- **判定**: `站位` 指出具体错位问题（不等于"无错位"）。
- **指令**: 命令玩家按建议调整站位。
...
## 1. 最终输出要求
- **禁止思考外露**: 不得在回复中出现任何思考过程、推理链、内心独白、`<|THINKING|>` / `[THINKING]` 等标签或 Markdown 思考块。**仅输出**最终一句。
- **极简原则**: 字数 **60 字以内**。严禁列 123，严禁复读快报原数据，直接说结论。
- **去技术化**: 严禁提及 Trigger、快报、审计指令、待追2星 等系统词汇。
- **纯正黑话**: 必须使用给定的绰号（如女枪、乌鸦、羊刀）。
- **原创性**: 基于当前[血量] 调整语气：40血以上是‘高冷嘲讽’，40血以下是‘歇斯底里’，20血以下是‘临终关怀’。
...
def build_coach_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--minimal-debug",
        action="store_true",
        help="开启后终端仅打印喂给大模型的极简版Prompt，关闭时打印完整战术快报",
    )
    ap.add_argument(
        "--pipeline-verbose",
        action="store_true",
    ...
    ap.add_argument(
        "--use-gemini",
        action="store_true",
        help="开启后通过 OpenRouter 切换使用 google/gemini-2.5-flash 模型（默认使用 openai/gpt-5.4）",
    )
    return ap
...
def build_coach_bundle(
    ...
    brief_for_terminal = build_matchbook_report(
        summary,
        core_chess_path=args.rag_core_chess.resolve(),
        summary_json_path=summary_path,
        ...
        no_rag=args.no_rag,
        minimal_for_llm=False,
    )
    
    brief_for_llm = build_matchbook_report(
        summary,
        core_chess_path=args.rag_core_chess.resolve(),
        summary_json_path=summary_path,
        lineup_v1_report=lineup_v1_report_str if not args.no_rag else None,
    ...
    return {
        "summary": summary,
        "brief": brief_for_llm,
        "brief_terminal": brief_for_terminal,
        "rag_block": rag_block,
...
def print_coach_bundle_preview(args: argparse.Namespace, bundle: Dict[str, Any]) -> None:
    ...
    # 极简模式开关，开启时只打印给大模型的 prompt
    minimal_debug = getattr(args, 'minimal_debug', False)
    ...
    if minimal_debug:
        ...
        print("\n--- [完整版 战术快报 (本地查看)] ---")
        brief = str(bundle.get("brief_terminal") or bundle.get("brief") or "")
        print("\n" + brief)
        print("------------------------------------\n")
