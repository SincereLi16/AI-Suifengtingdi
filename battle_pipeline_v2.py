# -*- coding: utf-8 -*-
"""
battle_pipeline_v2：一体化对局识别 + 装备栏竖条（equip_column）+ 仅模式 A（trait cross validate）。

相对 battle_pipeline.py：
- 增加 element_recog.equip_column_recog：左侧装备栏 ROI 模板匹配。
- 仅保留「主图 a + 辅图 b」聚合逻辑：与 trait_cross_validate 一致，无主图/副图模式 B。
- 输出：仅主图（-a / _a 或纯数字 stem）写出 {stem}_annotated.png 与 {stem}_summary.json；
  辅图不单独出图、不出 JSON。左侧图仅 Fightboard 标注；右侧 summary 分节：羁绊（含 TCV）、棋盘、备战、装备、局势。
  羁绊人数与名称来自同组多图聚合（与 trait_cross_validate 一致），终端输出各步骤细分耗时。
  本脚本不启用模块级 pkl 缓存（便于调试）；chess_recog 内建 embedding 向量库缓存仍可能读写 `.piece_db_cache`。
"""
...
DEFAULT_OUT = PROJECT_DIR / "battle_pipeline_v2_out"
...
def _summary_lines_v2_primary(
    *,
    ...
    lines: List[str] = []
    lines.append("battle_pipeline_v2 · 主图")
    lines.append("")
    lines.append("【羁绊】")
...
def _run_one_fightboard_profiled(
    *,
    ...
    print(
        f"  [{tag}] fightboard-2 chess_recog.run_recognition（血条检测+嵌入匹配+落盘中间结果）: "
...
def _run_one_preboard_profiled(
    *,
    ...
    try:
        ...
        n_samp = len(samples) if isinstance(samples, list) else 0
        print(
            f"  [{tag}] preboard-2 chess_recog.run_recognition（备战区 ROI）: "
...
def _ensure_fight_pre_runtime(
    ...
    if model_holder[0] is not None:
        return
    print("[V2] 初始化 fightboard / preboard 模型…")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="battle_pipeline_v2：fight+pre+player+equip_column + trait cross validate（仅主图出图）")
    ...
    if not need_fight:
        print("[V2] 当前输入无主图（如 -a），将仅跑 player/equip_column，不进行 fightboard 交叉验证。")

    if need_fight:
        print("[V2] 预加载 PyTorch（先于 Paddle OCR，避免 Windows DLL 冲突）…")
    ...
    print(f"[V2] 初始化完成 ({t0_all:.1f}s 起算)")
    ...
    with tempfile.TemporaryDirectory(prefix="bp_v2_tmp_") as td:
        tmp = Path(td)
        for image_path in images:
            ...
            if (src_w, src_h) != (target_w, target_h):
                ...
                print(
                    f"[V2] 尺寸对齐 {image_path.name}: {src_w}x{src_h} -> {target_w}x{target_h} "
                    f"耗时={t_n1 - t_n0:.4f}s"
                )

            run_fight = tcv._stem_matches_fightboard_suffix(stem, suffix)
            if run_fight:
                print(f"[V2] 处理 {image_path.name}（主图 · fight + pre + player + equip_column）…")
            else:
                print(f"[V2] 处理 {image_path.name}（辅图 · player + equip_column）…")
            ...
            print(f"  [V2] 本图合计耗时: {time.perf_counter() - t_img:.4f}s  ({image_path.name})")
    ...
    for stem in sorted(fight_by_stem.keys()):
        ...
        lines = _summary_lines_v2_primary(
            fight_json=fight_js,
            pre_json=pre_js,
