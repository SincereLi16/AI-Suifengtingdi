...
from project_paths import DEFAULT_OUT_BATTLE_PIPELINE_V3, PROJECT_ROOT

PROJECT_DIR = PROJECT_ROOT
DEFAULT_INPUT = PROJECT_DIR / "对局截图"
DEFAULT_OUT = DEFAULT_OUT_BATTLE_PIPELINE_V3
...
def _configure_pipeline_devices() -> Tuple[str, Dict[str, bool]]:
    ...
    print(f"[Pipeline] fightboard MobileNet torch device={dev} ({backend})")
    name, kwargs = pon._resolve_device("auto")
    print(f"[Pipeline] player_onnx OCR ort device={name}")
    return name, kwargs
...
def _ensure_fightboard_mobilenet_runtime() -> None:
    if _mobilenet_bundle_box[0] is not None:
        return
    print("[Pipeline] 初始化 fightboard_mobilenet（MobileNet + 装备模板）…")
    try:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="pipeline：fightboard_mobilenet + player_onnx + equip_column + TCV（仅主图出图）"
    )
    ...
    if not need_fight:
        print("[Pipeline] 当前输入无主图（如 -a），将仅跑 player/equip_column，不进行 fightboard 交叉验证。")
    else:
        print("[Pipeline] 预加载 fightboard MobileNet（先于 player_onnx OCR 初始化）…")
    ...
    print(
        "[Pipeline] equip_column 模板分层: "
        + ", ".join(f"{k}={int(v)}" for k, v in col_tier_counts.items())
        + f" | total={len(col_templates)} | block_workers={equip_col_bw}"
    )
    if args.serial_player:
        print("[Pipeline] player_onnx：已启用 --serial-player（fight+equip 并行结束后再跑 OCR）。")

    t0_all = time.perf_counter()
    print(f"[Pipeline] 初始化完成 ({t0_all:.1f}s 起算)")
    ...
    with tempfile.TemporaryDirectory(prefix="bp_v3_tmp_") as td:
        ...
        for image_path in images:
            ...
            if (src_w, src_h) != (target_w, target_h):
                ...
                print(
                    f"[Pipeline] 尺寸对齐 {image_path.name}: {src_w}x{src_h} -> {target_w}x{target_h} "
                    f"耗时={t_n1 - t_n0:.4f}s"
                )
            ...
            if run_fight:
                print(f"[Pipeline] 处理 {image_path.name}（主图 · fightboard + player + equip_column）…")
            else:
                print(f"[Pipeline] 处理 {image_path.name}（辅图 · 羁绊栏 player_onnx）…")
            ...
            print(f"  [Pipeline] 本图合计耗时: {img_total:.4f}s  ({image_path.name})")
            if run_fight:
                ...
                print(f"  [{image_path.name}] Pipeline阶段拆分:")
                if parallel_player:
